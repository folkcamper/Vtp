#! /Skel/bin/perl
my $VERSION='2017-08-09.01-27-47.EDT';
my $VERSION = '2017-08-09.01-26-08.EDT';

# start recording a image file, of a specified window,
...
my $imagepid;     # pid of the ffmpeg instance recording the window
my $soundpid;     # pid of the notice sound
my $prefix;       # string designating prefix of the recorded file
my $writepath;    # fully qualified directory to write the image ile
my $writefn;      # the filename of the writable image file.

### MAIN

if ( defined( $OPTS{'k'} ) && length( $ENV{'VTP_IMAGEPID'} ) ) {    # kill stored pid

   system("xmessage $ENV{'VTP_IMAGEPID'} $ENV{'SOUND_STOPRECORD'}") if $_DEBUG;

   system("kill $ENV{'VTP_IMAGEPID'}");
   ...
   $rctemplate->append( 'vtp_imagepid' => '' );    # empty the image pid

   # write the rc file

   singlequote( $rctemplate, 'vtp_imagegeom', 'vtp_imagegeom' );
   &printrc($rctemplate);
   ...
   unless ( length($geometry) ) {
      if ( length( $OPTS{'e'} ) ) {
         ...
         # $rctemplate->append( 'vtp_imagegeom' => $geometry ) ; # but don't persist
         $rctemplate->append( 'vtp_imageres' => '' );    # empty the image resolution
      } else {
   ...
   $rctemplate->append( 'vtp_imagegeom' => $geometry );
   $rctemplate->append( 'vtp_imageres'  => $OPTS{'r'} );    # empty the image resolution
...
$rctemplate->append( 'vtp_imagefile' => $writefn );
...
# my $imgcommand = "ffmpeg -thread_queue_size 512 -f pulse -ac 2 -i default -f x11grab $geometry -r 32 -acodec pcm_s16le -vcodec libx264 -preset ultrafast -crf 0 -y $writefn";

my $imgcommand = "ffmpeg -f x11grab $geometry -vframes 1 $writefn";

print "$imgcommand\n";

# $imagepid = fork { "exec" => "$imgcommand" };    # get arecord PID

$rctemplate->append( 'vtp_imagepid' => $imagepid );    # empty the image resolution
...
sub pdtwritepath {
   ...
   return ( $ENV{'HOME'} ) if ( length( $ENV{'VTP_IMAGERES'} ) + length( $OPTS{'r'} ) ) < 1;
...
sub setgetprefix {
   ...
   } elsif ( length( $ENV{'VTP_IMAGEPREFIX'} ) ) {
      $prefix = $ENV{'VTP_IMAGEPREFIX'};
      $rctemplate->append( 'vtp_imageprefix' => $ENV{'VTP_IMAGEPREFIX'} );
   } else {
      $prefix = 'image';
   }
...
sub dohelp {
   ...
   print "-w <windowid> -p <image_filename_prefix> -r <resolution_name> -e (configure from environment)\n\n";
   exit;
